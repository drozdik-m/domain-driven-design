...
public class ServicePipelineBuilder<TInput, TOutput>
{
    ...
    public ServicePipelineBuilder<TInput, TOutput> Add<TPipelineBehaviour>()
        where TPipelineBehaviour : IPipelineBehavior<TInput, TOutput>
    {
    ...
    /// <returns>This for chaining.</returns>
    public ServicePipelineBuilder<TInput, TOutput> Add(Type pipelineType)
    {
        // Validate the pipeline type
        ArgumentNullException.ThrowIfNull(pipelineType);
        if (!typeof(IPipelineBehavior<TInput, TOutput>).IsAssignableFrom(pipelineType))
        {
            throw new ArgumentException($"Type {pipelineType.Name} does not implement {nameof(IPipelineBehavior<TInput, TOutput>)}", nameof(pipelineType));
        }
    ...
    public IPipelineBehavior<TInput, TOutput> Build(IServiceProvider serviceProvider)
    {

...
- [ ] **`IntIdentity` / `StringIdentity` are unusable as documented** — both declare `where TSelf : ..., new()` (`Identities/Primitive/IntIdentity.cs:5`, `StringIdentity.cs:5`), unlike `GuidIdentity`. A derived type with a primary constructor (e.g. `OrderId(int key) : IntIdentity<OrderId>(key)`, as shown in the skill docs) has no parameterless constructor and **fails the `new()` constraint → CS0310, won't compile**. Only `GuidIdentity` is exercised today, so it went unnoticed. Fix: drop `, new()` from both.
